...
manuf_name, which is the first three letters of the manufacturers name.
...
full_data <-
  ...
  mutate(
    week_end = lubridate::ymd(week_end),
    sales = units * price,
    manuf_name = substr(brand_descr, 1, 3),
    across(c(feature, display), ~ifelse(is.na(.), FALSE, TRUE))
  ) %>% 
  select(
    -prmult, 
    -parent_code, 
...
drive_put(file.path(getwd(), "Data", paste(data_name, sep = "", ".csv")), path = paste("Category Captainship - Empirical Generalization/Cleaned Data/", data_name, "/", sep = ""))
